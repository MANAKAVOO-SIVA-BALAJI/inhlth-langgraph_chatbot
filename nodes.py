...
def call_llm(state: AgentState):
    logger.info("call_llm is executing..")
    response = llm_bind_tool.invoke([system_query_prompt_format]+state["messages"])
    logger.info(f"call_llm: {response.content}")
...
def call_tool(state: AgentState):
    ...
    for call in last_ai_message.tool_calls:
        ...
        logger.info(f"Tool {tool_name} executed with input: {tool_input}")
...
def data_analyser(state: AgentState):
    ...
    logger.info(f"data_analyser: {response.content}")
    return {"messages": state["messages"] + [response]}
