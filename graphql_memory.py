...
class HasuraMemory:
    ...
    def deserialize_history(self, history):
        ...
        for record in history:
            ...
            if isinstance(msg, list):
                for m in msg:
                    deserialized.extend(self.convert_message(m))
            elif isinstance(msg, dict):
                deserialized.extend(self.convert_message(msg))
    ...
    def save_messages(
        ...
        for i, msg in enumerate(messages):
            ...
            sender_type = "user" if isinstance(msg, HumanMessage) else "ai"

            node = nodes[i] if i < len(nodes) else None
            if node in ["data_analyser", "general_response", "clarify"]:
                sender_type = "final_response"

            created_at = time[i] if i < len(time) else None
            ...
            if sender_type == "user":
                cache_messages.append(HumanMessage(content=serialized_msg["content"]))
            elif sender_type == "final_response":
                cache_messages.append(AIMessage(content=serialized_msg["content"]))

            step += 1

        if not objects:
            print("[SAVE_MESSAGES] No messages to insert.")
            return
        ...
        try:
            ...
            data = response.json()
            if "errors" in data:
                logger.error(f"[SAVE_MESSAGES] Error: {data['errors']}")
                return
            memory_cache.store_message(self.user_id, cache_messages)
        except Exception as e:
            logger.error(f"[SAVE_MESSAGES] Error inserting into Hasura: {e}")
    ...
    def get_messages(self, config: Dict[str, Any]) -> List:
        ...
        try:
            response = requests.post(
                self.hasura_url, json=payload, headers=self.headers, timeout=10
            )
            ...
            logger.error(f"[get_messages] Request error: {e}")
            return []
        except Exception as e:
            logger.error(f"[get_messages] Unexpected error: {e}")
            return []
    ...
    def run_query(self, query, variables=None):
        try:
            ...
            print("[run_query] Timeout calling Hasura.")
            return {}
        except RequestException as e:
            print(f"[run_query] Request error: {e}")
            return {}
        except Exception as e:
            print(f"[run_query] Unexpected error: {e}")
            return {}
...
#                 for m in msg:
#                     deserialized.extend(self.convert_message(m))
#             # If it's a single message dictionary
#             elif isinstance(msg, dict):
#                 deserialized.extend(self.convert_message(msg))
